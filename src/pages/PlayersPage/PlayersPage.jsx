...
// Resolve teamId string → full team object once, outside the component
const enrichedPlayers = playersData.map((player) => ({
  ...player,
  team: teamsData[player.teamId] ?? null,
}));

const PlayersPage = () => {
  // Build filter tabs — only competitions that have at least one player
  const filters = useMemo(() => {
    const usedIds = new Set(
      enrichedPlayers.flatMap((p) => Object.keys(p.stats?.byCompetition ?? {})),
    );
    ...
    return [{ value: FILTER_ALL, label: "Todos" }, ...competitionFilters];
  }, []);

  const [activeFilter, setActiveFilter] = useState(FILTER_ALL);

  // Filter players by the selected competition
  const players = useMemo(() => {
    if (activeFilter === FILTER_ALL) return enrichedPlayers;

    return enrichedPlayers.filter(
      (p) => p.stats?.byCompetition?.[activeFilter] != null,
    );
  }, [activeFilter]);
