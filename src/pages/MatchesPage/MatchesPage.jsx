...
// ============================================
// MatchesPage.jsx (with dual filtering + round subdivision)
// ============================================

// NEW FEATURE: Round Subdivision
// ─────────────────────────────────────
/**
 * After the competition + status filters run, matches are grouped into
 * their Round ("Jornada N"), and rendered as separate sections:
 *
 *   Jornada 5
 *     - match, match, match
 *   Jornada 6
 ...
 * ───────────────────────────────
 * Ideally each match object has an explicit `round` field (e.g. `round: 6`).
 * That's the reliable long-term source of truth — add it to your data
 * whenever possible.
 *
 * FALLBACK: if `match.round` is missing, we extract it from the `info`
 * string (e.g. "CFNM 2026 - Jornada 6" → 6) using a regex. This keeps
 * older match objects working without a data migration, but it's a
 * fallback, not the recommended long-term approach — free text is easy
 * to accidentally break (typos, different phrasing, etc).
 *
 * Matches where no round can be determined land in an "Outros Jogos"
 * bucket at the end, instead of silently disappearing.
 */
...
const MATCH_FILTERS = [
  ...
  { value: "canceled", label: "Não Terminados" },
  { value: "postponed", label: "Adiados" },
];

// ─── Status priority for sorting (used for the flat list / non-round sort) ──
const statusPriority = {
  live: 1,
  upcoming: 2,
  finished: 3,
...
// ─── NEW: Round extraction helpers ──────────────────────────────
// ─────────────────────────────────────
/**
 * Matches "Jornada" (Portuguese for "matchday/round") followed by a number,
 * case-insensitively, anywhere in the info string.
 * e.g. "CFNM 2026 - Jornada 6" → captures "6"
 */
const ROUND_REGEX = /Jornada\s*(\d+)/i;

// Given a match, figure out its round LABEL (what gets displayed as a header)
const getRoundLabel = (match) => {
  // 1. Preferred path: explicit round field
  if (match.round !== undefined && match.round !== null) {
    return typeof match.round === "number"
      ? `Jornada ${match.round}`
      : match.round; // already a formatted string
  }

  // 2. Fallback: try to extract "Jornada N" from the info string
  const found = match.info?.match(ROUND_REGEX);
  if (found) {
    return `Jornada ${found[1]}`;
  }

  // 3. Nothing found — bucket it so it's still visible, not silently dropped
  return "Outros Jogos";
};

// Given a round label, extract just the number for sorting purposes.
// Returns null for labels with no number (e.g. "Outros Jogos"),
// which we then push to the end of the list.
const extractRoundNumber = (label) => {
...
const MatchesPage = ({ matches }) => {
  ...
  const initialFilter =
    urlFilter &&
    ["live", "finished", "upcoming", "canceled", "postponed"].includes(
      urlFilter,
    )
      ? urlFilter
      : "all";
  ...
  const competitionFilters = useMemo(() => {
    ...
    // Only keep competitions that have at least one match
    const competitionIdsWithMatches = new Set(
      matches.map((match) => match.competitionId),
    );

    const options = competitionsData
      .filter((comp) => competitionIdsWithMatches.has(comp.id))
      .map((comp) => ({
        value: comp.id,
        label: comp.name,
      }));
    ...
    return [allOption, ...options];
  }, [matches]); // ← was [], now depends on matches

  // ── Step A + B: competition filter, then status filter ─────────
  const filteredMatches = useMemo(() => {
    const byCompetition =
      activeCompetition === "all"
        ? matches
    ...
    return byStatus.slice().sort(matchSorter);
  }, [matches, activeCompetition, activeFilter]);

  // ── NEW Step C: group filteredMatches into rounds ───────────────
  // ─────────────────────────────────────────────────────────────
  /**
   * WHY A SEPARATE useMemo?
   * Grouping is its own "expensive" step — no reason to recompute it
   * unless filteredMatches itself changed.
   *
   * OUTPUT SHAPE:
   *   [ ["Jornada 5", [match, match]], ["Jornada 6", [match]], ... ]
   *
   * We use an array of [label, matches] pairs (not a plain object) so we
   * can control the ORDER of rounds explicitly — plain object key order
   * for numeric-looking strings isn't something you want to rely on.
   */
  const groupedByRound = useMemo(() => {
    ...
    // Sort matches WITHIN each round chronologically (earliest first).
    // This is intentionally simpler than matchSorter — inside a single
    // round you want fixture order, not status-priority reshuffling.
    for (const roundMatches of groups.values()) {
    ...
    // Sort the ROUNDS themselves numerically ascending.
    // Labels with no number (e.g. "Outros Jogos") go last.
    return [...groups.entries()].sort(([labelA], [labelB]) => {
      const numA = extractRoundNumber(labelA);
      const numB = extractRoundNumber(labelB);
  ...
  return (
    <div className="matches-page-container">
      ...
      {/* ── NEW: rounds rendered as subsections ─────────────────── */}
      {groupedByRound.length > 0 ? (
        groupedByRound.map(([roundLabel, roundMatches]) => (
          <section key={roundLabel} className="round-section">
            <h3 className="round-heading">{roundLabel}</h3>
            <MatchesGrid matches={roundMatches} />
          </section>
        ))
      ) : (
        <div className="empty-state">
